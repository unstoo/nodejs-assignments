...
- `list.js` - implement function that prints an array with all filenames from `files` folder into console (if `files` folder doesn't exists `Error` with message `FS operation failed` must be thrown)
